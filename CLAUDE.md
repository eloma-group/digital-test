# EG Digital Landing — Project Rules
...
### Mobile-first banner pattern

For hero/banner sections that use `position: sticky` with a large wordmark:

```css
/* Mobile: panel at bottom, gradient fade over wordmark */
@media (max-width: 540px) {
  .banner-right {
    ...
    bottom: 0 !important;
    height: 60% !important;
    background: linear-gradient(to bottom, transparent 0%, rgba(243,240,234,0.85) 20%, rgba(243,240,234,1) 38%) !important;
...
- **All headings are uppercase** with `letterSpacing: '-0.04em'` to `-0.05em` and `fontWeight: 900`.
...
```ts
// InnerHero background watermark
fontSize: 'clamp(180px, 26vw, 380px)'

// InnerHero / PageCTA main headline
fontSize: 'clamp(52px, 10.5vw, 136px)'

// Section heading
fontSize: 'clamp(52px, 11vw, 152px)'

// Editorial giant sub-heading
fontSize: 'clamp(60px, 14.5vw, 208px)'
...
// Eyebrow label
fontSize: 'clamp(10px, 0.75vw, 12px)', fontWeight: 800, letterSpacing: '2.5px'
```
...
### Pattern: scroll-driven value (correct)

```ts
// ✅ Compositor-safe — runs on the compositor thread
const y = useTransform(scrollYProgress, [0, 1], [0, -120])
<motion.div style={{ y, opacity }} />
```

```ts
// ❌ Forces main-thread recalc every frame
useEffect(() => {
...
### Pattern: ambient animation (correct)
...
.shape { will-change: transform; animation: float 18s linear infinite; }
...
- Lenis (smooth scroll)
- Lucide React (icons)
- Inline styles + scoped `<style>` blocks (no CSS modules or Tailwind)
